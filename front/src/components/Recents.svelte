...
<article>
    ...
    <div class="players">
        ...
        {:then result}
            {#if result.length === 0}
                <span>
                    No files have been converted within the past 24 hours.
                </span>
            {:else}
                {#each result as file}
                    <AudioPlayer display={file} />
                {/each}
            {/if}
        {:catch error}
            <img
            ...
            <span class="error-text">
                An error occurred fetching the list of recently converted files.
            </span>
        {/await}
    </div>
</article>
...
<style>
    ...
    .error-icon {
        width: 2.5em;
        height: auto;
        filter: invert(22%) sepia(33%) saturate(5749%) hue-rotate(345deg)
